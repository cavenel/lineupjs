...
export class RankingBuilder {
     ...
     */
    impose(label, numberColumn, colorColumn) {
        this.columns.push({
            desc: (data) => {
                const base = data.getColumns().find((d) => d.label === numberColumn || d.column === numberColumn);
     ...
     */
    scripted(label, code, ...numberColumns) {
        this.columns.push({
