...
export default class LocalDataProvider extends ACommonDataProvider {
    ...
    createSorter(ranking, filter, needsFiltering, needsGrouping, needsSorting) {
        ...
        const toGroup = groupCriteria.length === 1
            ? (r) => joinGroups([groupCriteria[0].group(r, groupCaches[0] ? groupCaches[0](r.i) : undefined)])
            : (r) => joinGroups(groupCriteria.map((c, i) => c.group(r, groupCaches[i] ? groupCaches[i](r.i) : undefined)));
        if (needsFiltering) {
            // filter, group, sort
        ...
        for (const before of ranking.getGroups()) {
            ...
            if (!needsGrouping) {
                ...
                if (!lookups) {
                    maxDataIndex = order.reduce((a, b) => Math.max(a, b), maxDataIndex);
                    continue;
                }
                // sort
    ...
    sortGroup(g, i, ranking, lookups, groupLookup, singleGroup, maxDataIndex) {
        const group = g.group;
    ...
    sortGroups(groups, groupLookup, enforceSorting) {
        // sort groups
        if (groupLookup) {
    ...
    sort(ranking, dirtyReason) {
        ...
        const needsGrouping = needsFiltering ||
        ...
        const needsGroupSorting = needsGrouping ||
    ...
    search(search, col) {
        ...
        const f = typeof search === 'string'
            ? (v) => v.toLowerCase().indexOf(search) >= 0
