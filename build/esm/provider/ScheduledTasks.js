...
export class ScheduleRenderTasks extends ARenderTasks {
    constructor() {
        super(...arguments);
        this.cache = new Map();
        this.tasks = new TaskScheduler();
        this.workers = new WorkerTaskScheduler(createWorkerBlob());
    }
    setData(data) {
        this.data = data;
        this.cache.clear();
    ...
    dirtyRanking(ranking, type) {
        ...
        switch (type) {
            case 'group':
                checker = cols.map((col) => (key) => key.startsWith(`${col.id}:a:group`));
                break;
            case 'summary':
                checker = cols.map((col) => (key) => key.startsWith(`${col.id}:b:summary`) || key.startsWith(`${col.id}:a:group`));
                break;
            case 'data':
            default:
                checker = cols.map((col) => (key) => key.startsWith(`${col.id}:`));
                break;
        }
        for (const key of Array.from(this.cache.keys()).sort()) {
    ...
    preCompute(ranking, groups, maxDataIndex) {
        if (groups.length === 0) {
            return;
        }
        const cols = ranking.flatColumns;
        if (groups.length === 1) {
            ...
            for (const col of cols) {
                ...
                this.chainCopy(`${col.id}:a:group:${group.name}`, this.cache.get(`${col.id}:b:summary`), (v) => ({ group: v.summary, summary: v.summary, data: v.data }));
                if (isNumberColumn(col)) {
                    this.chainCopy(`${col.id}:a:group:${group.name}:raw`, this.cache.get(`${col.id}:b:summary:raw`), (v) => ({ group: v.summary, summary: v.summary, data: v.data }));
                }
            }
            return;
        }
    ...
    copyData2Summary(ranking) {
        for (const col of ranking.flatColumns) {
            ...
            this.chainCopy(`${col.id}:b:summary`, this.cache.get(`${col.id}:c:data`), (data) => ({
                summary: data,
                data,
            }));
            if (isNumberColumn(col)) {
                this.chainCopy(`${col.id}:b:summary:raw`, this.cache.get(`${col.id}:c:data:raw`), (data) => ({
                    summary: data,
                    data,
                }));
            }
    ...
    copyCache(col, from) {
        ...
        for (const key of Array.from(this.cache.keys()).sort()) {
            ...
            this.chainCopy(chainKey, this.cache.get(key), (data) => data);
        }
    }
    ...
    groupRows(col, group, key, compute) {
    ...
    groupBoxPlotStats(col, group, raw) {
        return this.chain(`${col.id}:a:group:${group.name}${raw ? ':braw' : ':b'}`, this.summaryBoxPlotStats(col, raw), ({ summary, data }) => {
            ...
            if (this.valueCacheData.has(key) && group.order.length > 0) {
                // web worker version
                return () => this.workers
                    .pushStats('boxplotStats', {}, key, this.valueCacheData.get(key), `${ranking.id}:${group.name}`, group.order)
    ...
    groupNumberStats(col, group, raw) {
        return this.chain(`${col.id}:a:group:${group.name}${raw ? ':raw' : ''}`, this.summaryNumberStats(col, raw), ({ summary, data }) => {
            ...
            if (this.valueCacheData.has(key) && group.order.length > 0) {
                // web worker version
                return () => this.workers
                    .pushStats('numberStats', { numberOfBins: summary.hist.length, domain: this.resolveDomain(col, raw) }, key, this.valueCacheData.get(key), `${ranking.id}:${group.name}`, group.order)
                    .then((group) => ({ group, summary, data }));
            }
    ...
    groupCategoricalStats(col, group) {
        return this.chain(`${col.id}:a:group:${group.name}`, this.summaryCategoricalStats(col), ({ summary, data }) => {
            ...
            if (this.valueCacheData.has(col.id) && group.order.length > 0) {
                // web worker version
                return () => this.workers
                    .pushStats('categoricalStats', { categories: col.categories.map((d) => d.name) }, col.id, this.valueCacheData.get(col.id), `${ranking.id}:${group.name}`, group.order)
                    .then((group) => ({ group, summary, data }));
            }
    ...
    groupDateStats(col, group) {
        ...
        return this.chain(key, this.summaryDateStats(col), ({ summary, data }) => {
            ...
            if (this.valueCacheData.has(col.id) && group.order.length > 0) {
                // web worker version
                return () => this.workers
                    .pushStats('dateStats', { template: summary }, col.id, this.valueCacheData.get(col.id), `${ranking.id}:${group.name}`, group.order)
                    .then((group) => ({ group, summary, data }));
            }
    ...
    groupStringStats(col, group) {
        ...
        return this.chain(key, this.summaryStringStats(col), ({ summary, data }) => {
            ...
            if (this.valueCacheData.has(col.id) && group.order.length > 0) {
                // web worker version
                return () => this.workers
                    .pushStats('stringStats', { topN: this.options.stringTopNCount }, col.id, this.valueCacheData.get(col.id), `${ranking.id}:${group.name}`, group.order)
                    .then((group) => ({ group, summary, data }));
            }
            return this.stringStatsBuilder(group.order, col, topN, (group) => ({ group, summary, data }));
        });
    }
    summaryBoxPlotStats(col, raw, order) {
        return this.chain(`${col.id}:b:summary${raw ? ':braw' : ':b'}`, this.dataBoxPlotStats(col, raw), (data) => {
            ...
            if (this.valueCacheData.has(key)) {
                // web worker version
                return () => this.workers
                    .pushStats('boxplotStats', {}, key, this.valueCacheData.get(key), ranking.id, order ? order.joined : ranking.getOrder())
                    .then((summary) => ({ summary, data }));
            }
            return this.boxplotBuilder(order ? order : ranking.getOrder(), col, raw, (summary) => ({ summary, data }));
        });
    }
    summaryNumberStats(col, raw, order) {
        return this.chain(`${col.id}:b:summary${raw ? ':raw' : ''}`, this.dataNumberStats(col, raw), (data) => {
            ...
            if (this.valueCacheData.has(key)) {
                // web worker version
                return () => this.workers
                    .pushStats('numberStats', { numberOfBins: data.hist.length, domain: this.resolveDomain(col, raw) }, key, this.valueCacheData.get(key), ranking.id, order ? order.joined : ranking.getOrder())
                    .then((summary) => ({ summary, data }));
            }
    ...
    summaryCategoricalStats(col, order) {
        return this.chain(`${col.id}:b:summary`, this.dataCategoricalStats(col), (data) => {
            const ranking = col.findMyRanker();
            if (this.valueCacheData.has(col.id)) {
                // web worker version
                return () => this.workers
                    .pushStats('categoricalStats', { categories: col.categories.map((d) => d.name) }, col.id, this.valueCacheData.get(col.id), ranking.id, order ? order.joined : ranking.getOrder())
                    .then((summary) => ({ summary, data }));
            }
            return this.categoricalStatsBuilder(order ? order : ranking.getOrder(), col, (summary) => ({ summary, data }));
        });
    }
    summaryDateStats(col, order) {
        return this.chain(`${col.id}:b:summary`, this.dataDateStats(col), (data) => {
            const ranking = col.findMyRanker();
            if (this.valueCacheData.has(col.id)) {
                // web worker version
                return () => this.workers
                    .pushStats('dateStats', { template: data }, col.id, this.valueCacheData.get(col.id), ranking.id, order ? order.joined : ranking.getOrder())
                    .then((summary) => ({ summary, data }));
            }
            return this.dateStatsBuilder(order ? order : ranking.getOrder(), col, data, (summary) => ({ summary, data }));
        });
    }
    summaryStringStats(col, order) {
        return this.chain(`${col.id}:b:summary`, this.dataStringStats(col), (data) => {
            const ranking = col.findMyRanker();
            if (this.valueCacheData.has(col.id)) {
                // web worker version
                return () => this.workers
                    .pushStats('stringStats', { topN: this.options.stringTopNCount }, col.id, this.valueCacheData.get(col.id), ranking.id, order ? order.joined : ranking.getOrder())
                    .then((summary) => ({ summary, data }));
            }
    ...
    dataBoxPlotStats(col, raw) {
        ...
        if (this.valueCacheData.has(valueCacheKey) && this.data.length > 0) {
            // use webworker
            return this.cached(key, false, () => this.workers.pushStats('boxplotStats', {}, valueCacheKey, this.valueCacheData.get(valueCacheKey)));
        }
    ...
    dataStringStats(col) {
        return this.cached(`${col.id}:c:data`, false, this.stringStatsBuilder(null, col));
    }
    dataDateStats(col) {
        return this.cached(`${col.id}:c:data`, false, this.dateStatsBuilder(null, col));
    }
    sort(ranking, group, indices, singleCall, maxDataIndex, lookups) {
