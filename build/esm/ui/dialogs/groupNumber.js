...
export default function groupNumber(col, node, dialog) {
  ...
  `);
    const threshold = node.querySelector(`#${dialog.idPrefix}N1`);
    const bins = node.querySelector(`#${dialog.idPrefix}N2`);
    forEach(node, 'input[name=threshold]', (d) => {
    ...
    return {
        ...
        submit() {
            const isThreshold = node.querySelector('input[name=threshold]:checked').value === 'threshold';
            if (isThreshold) {
