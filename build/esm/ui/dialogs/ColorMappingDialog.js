...
export default class ColorMappingDialog extends ADialog {
    ...
    createTemplate(id, wrapped) {
        ...
        h += `<div>`;
        {
            ...
            const renderColor = (d) => `<label class="${cssClass('checkbox-color')}">
        ...
        <span class="${cssClass('color-custom')}"><input type="color" name="solid" list="${id}L" value="${current instanceof SolidColorFunction ? this.ctx.sanitize(current.color) : DEFAULT_COLOR}" ${refColor && !has ? '' : 'disabled'}></span>
      </label>`;
        }
        h += '</div>';
    ...
    <label class="${cssClass('checkbox')}">
      ...
      <span>Discrete&nbsp;<input type="number" id="${id}KQS_D" min="2" step="1" style="width: 3em" value="${wrapped instanceof QuantizedColorFunction ? `${wrapped.steps}"` : '5" disabled'}>&nbsp; steps</span>
        ...
        <span class="${cssClass('color-custom')}">
          <input type="color" name="divergingm1" list="${id}L" ${!isCustom ? 'disabled' : `value="${this.ctx.sanitize(entries[0].color)}"`}>
          <input type="color" name="diverging0" list="${id}LW" ${!isCustom ? 'disabled' : `value="${this.ctx.sanitize(entries[1].color)}"`}>
          <input type="color" name="diverging1" list="${id}L" ${!isCustom ? 'disabled' : `value="${this.ctx.sanitize(entries[2].color)}"`}>
        </span>
      </label>`;
        }
        h += '</div>';
    ...
    render(node, value) {
        ...
        const toggles = Array.from(node.querySelectorAll('strong[data-toggle]'));
        ...
        this.forEach('input[name=color]', (d) => {
        ...
        this.forEach(`.${cssClass('color-custom')} input[type=color]`, (d) => {
            d.onchange = () => {
                const item = d.parentElement.previousElementSibling;
                updateColor(item);
            };
        });
        // sync and apply
    ...
    updateGradients(steps) {
        this.forEach(`span[data-c]`, (d) => {
...
function toColor(input, node) {
    switch (input.value) {
        ...
        case 'custom:sequential': {
            const s0 = node.querySelector('input[name=interpolate0]').value;
            const s1 = node.querySelector('input[name=interpolate1]').value;
        ...
        case 'custom:diverging': {
            const dm1 = node.querySelector('input[name=divergentm1]').value;
            const d0 = node.querySelector('input[name=divergent0]').value;
            const d1 = node.querySelector('input[name=divergent1]').value;
            return new CustomColorMappingFunction([
                { color: dm1, value: 0 },
                { color: d0, value: 0.5 },
                { color: d1, value: 1 },
...
function gradient(interpolate, steps = 2) {
    if (steps <= 1) {
...
function steppedGradient(color, count = 2) {
    if (count === 1) {
        return `${color(0)}`;
    }
    let r = `linear-gradient(to right`;
