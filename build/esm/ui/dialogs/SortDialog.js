...
export default class SortDialog extends ADialog {
    constructor(column, groupSortBy, dialog, ctx) {
        super(dialog, {
...
function sortOrder(node, column, idPrefix, groupSortBy = false) {
    ...
    const updateDisabled = (disable) => {
        forEach(node, 'input:not([name=sortorder]), select, textarea', (d) => {
            d.disabled = disable;
        });
    };
    updateDisabled(order.asc === undefined);
    forEach(node, 'input[name=sortorder]', (n) => {
    ...
    return {
        ...
        submit() {
            const asc = node.querySelector('input[name=sortorder]:checked').value;
            const priority = Number.parseInt(node.querySelector(`#${id}P`).value, 10) - 1;
            if (groupSortBy) {
