...
export default class CategoricalColorMappingDialog extends ADialog {
    ...
    build(node) {
      ...
      ${this.column.categories
        ...
        Array.from(node.querySelectorAll(`label.${cssClass('checkbox')}.${cssClass('dialog-color-table-entry')}`)).forEach((n, i) => {
        ...
        this.findInput('select').onchange = (evt) => {
            const scheme = sets[evt.currentTarget.value];
            if (!scheme) {
                return;
            }
            this.forEach('[data-cat]', (n, i) => {
    ...
    submit() {
        const cats = this.column.categories;
        const map = new Map();
        this.forEach('input[data-cat]', (n, i) => {
            const cat = cats[i];
