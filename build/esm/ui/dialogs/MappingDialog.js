...
export default class MappingDialog extends ADialog {
    ...
    build(node) {
        ...
        ${others.length > 0
            ? `<optgroup label="Copy From">${others
                .map((d) => `<option value="copy_${d.id}">${this.dialog.sanitize(d.label, d.desc.labelAsHTML === true)}</option>`)
                .join('')}</optgroup>`
            : ''}
      </select>
      </div>
        ...
        const g = node.querySelector(`.${cssClass('dialog-mapper-details')} > g`);
        this.forEach(`.${cssClass('dialog-mapper-details')} rect`, (d) => (d.onclick = (evt) => {
            evt.preventDefault();
            evt.stopPropagation();
        ...
        this.forEach(`#${this.idPrefix}min, #${this.idPrefix}max`, (d, i) => {
            d.onchange = () => {
    ...
    updateDomainWarning() {
        const warningNode = this.node.querySelector(`.${cssClass('dialog-mapper-warning')}`);
    ...
    createMappings() {
        ...
        const g = this.node.querySelector(`.${cssClass('dialog-mapper-details')} > g`);
        const domain = this.scale.domain;
    ...
    update() {
        ...
        this.forEach(`input[type=number]`, (d, i) => {
            d.value = round(this.rawDomain[i], 3).toString();
        });
    }
    updateLines(scale = this.scale) {
        this.forEach(`.${cssClass('dialog-mapper-details')}  > g > line[x1]`, (d) => {
    ...
    copyMapping(columnId) {
        const r = this.column.findMyRanker();
        if (!r) {
            return;
        }
        const ref = r.find(columnId);
