...
export default class ChangeRendererDialog extends ADialog {
    ...
    build(node) {
      ...
      ${item
            .sort(byName)
            .map((d) => ` <label class="${cssClass('checkbox')}"><input type="radio" name="renderer" value="${s(d.type)}" ${current === d.type ? 'checked' : ''}><span>${s(d.label)}</span></label>`)
            .join('')}
      <strong>Group Visualization</strong>
      ${group
            .sort(byName)
            .map((d) => ` <label class="${cssClass('checkbox')}"><input type="radio" name="group" value="${s(d.type)}" ${currentGroup === d.type ? 'checked' : ''}><span>${s(d.label)}</span></label>`)
            .join('')}
      <strong>Summary Visualization</strong>
      ${summary
            .sort(byName)
            .map((d) => ` <label class="${cssClass('checkbox')}"><input type="radio" name="summary" value="${s(d.type)}" ${currentSummary === d.type ? 'checked' : ''}><span>${s(d.label)}</span></label>`)
            .join('')}
    `);
