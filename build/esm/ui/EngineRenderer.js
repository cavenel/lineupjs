...
class EngineRenderer extends AEventDispatcher {
    constructor(data, parent, options) {
        ...
        this.updateAbles = [];
        this.zoomFactor = 1;
        this.enabledHighlightListening = false;
        ...
        this.ctx = {
            ...
            getPossibleRenderer: (col) => getPossibleRenderer(col, this.options.renderers, this.options.canRender),
            colWidth: (col) => (!col.isVisible() ? 0 : col.getWidth()),
            caches: {
                toolbar: new Map(),
        ...
        {
            // selection indicator
            const body = this.table.node.querySelector(`.${engineCssClass('body')}`);
    ...
    initProvider(data) {
        ...
        data.on(`${DataProvider.EVENT_ADD_RANKING}.body`, (ranking) => {
            this.addRanking(ranking);
        });
        data.on(`${DataProvider.EVENT_REMOVE_RANKING}.body`, (ranking) => {
            this.removeRanking(ranking);
        });
        data.on(`${DataProvider.EVENT_GROUP_AGGREGATION_CHANGED}.body`, (ranking) => {
        ...
        data.on(`${DataProvider.EVENT_JUMP_TO_NEAREST}.body`, (indices) => {
    ...
    update(rankings = this.rankings) {
        ...
        const heightsFor = (ranking, data) => {
            if (this.options.dynamicHeight) {
                ...
                const f = (v, d) => (typeof v === 'number' ? v : v(d));
                if (impl) {
                    return {
                        defaultHeight: round2(this.zoomFactor * impl.defaultHeight),
                        height: (d) => round2(this.zoomFactor * f(impl.height, d)),
                        padding: (d) => round2(this.zoomFactor * f(impl.padding, d)),
                    };
                }
            }
        ...
        for (const r of rankings) {
            ...
            const topNGetter = (group) => this.data.getTopNAggregated(r.ranking, group);
    ...
    setHighlightToNearest(dataIndices, scrollIntoView) {
        if (this.rankings.length === 0) {
            return false;
        }
