...
export default class MultiLevelRenderColumn extends RenderColumn {
    ...
    matchChildren(wrapper, children) {
        function matches(col, i) {
            //do both match?
            const n = wrapper.children[i];
        ...
        const lookup = new Map(Array.from(wrapper.children).map((n, i) => [n.dataset.colId, { node: n, summary: this.summaries[i] }]));
    ...
    updateHeader(node) {
        ...
        const wrapper = node.getElementsByClassName(cssClass('nested'))[0];
        if (!wrapper) {
            return r; // too early
        }
