...
export default class CategoricalCellRenderer {
    constructor() {
...
function hist(col, showLabels, sanitize) {
    const createBin = (c, color) => {
        return `<div class="${cssClass('histogram-bin')}" title="${sanitize(c.label)}: 0" data-cat="${sanitize(c.name)}" ${showLabels ? `data-title="${sanitize(c.label)}"` : ''}><div style="height: 0; background-color: ${color}"></div></div>`;
    };
    return {
        template: (() => {
            const mapping = col.getColorMapping();
            const bins = col.categories.map((c) => createBin(c, mapping.apply(c))).join('');
            return `<div class="${cssClass('histogram')} ${col.categories.length > DENSE_HISTOGRAM ? cssClass('dense') : ''}">${bins}`; // no closing div to be able to append things
        })(),
        matchBins: (n) => {
            ...
            const bins = Array.from(n.querySelectorAll('[data-cat]'));
            let lastBin = null;
            let changed = false;
...
export function interactiveHist(col, node) {
    ...
    const markFilter = (bin, cat, value) => {
        // update filter highlight eagerly for better user feedback
        const inner = bin.firstElementChild;
