...
export default class DateHistogramCellRenderer {
    constructor() {
...
export function createDateFilter(col, parent, context, livePreviews) {
    ...
    const filter = col.getFilter();
    let domain = [isFinite(filter.min) ? filter.min : 0, isFinite(filter.max) ? filter.max : 100];
    ...
    const summaryNode = parent.firstElementChild;
...
function getHistDOMRenderer(col) {
    ...
    const render = (n, stats, unfiltered) => {
...
function createFilterInfo(col, domain, filter = col.getFilter()) {
...
function createFilterContext(col, context, domain) {
    ...
    const percent = (v) => clamp(Math.round((100 * (v - domain[0])) / (domain[1] - domain[0])));
    ...
    return {
        ...
        edit: (value, attachment, type, otherValue) => {
            return new Promise((resolve) => {
                ...
                const dialog = new InputDateDialog(dialogCtx, (d) => resolve(d == null ? NaN : shiftFilterDateDay(d.getTime(), type)), {
