...
export default class SetCellRenderer {
    constructor() {
        this.title = 'Matrix';
    }
    canRender(col) {
    ...
    static createDOMContext(col, sanitize) {
        ...
        for (const cat of categories) {
            templateRows += `<div class="${cssClass('heatmap-cell')}" title="${sanitize(cat.label)}" style="background-color: ${mapping.apply(cat)}"></div>`;
        }
        return {
    ...
    createSummary(col, context) {
        ...
        for (const cat of categories) {
            templateRows += `<div class="${cssClass('heatmap-cell')}" title="${context.sanitize(cat.label)}"${labels ? ` data-title="${context.sanitize(cat.label)}"` : ''} style="background-color: ${mapping.apply(cat)}"></div>`;
        }
        templateRows += '</div>';
        return {
