...
export default class ViolinPlotCellRenderer {
    constructor() {
...
function createViolinTemplate(width, isMapped) {
    ...
    return `<div title="">
      <svg class="${cssClass('violin')}"
        ...
        <line x1="0" x2="0" y1="${h / 2 - h * 0.25}" y2="${h / 2 + h * 0.25}" class="${cssClass('violin-median')}"></line>
      </svg>
      ${mappedHelper}
    </div>`;
}
...
function renderViolin(col, n, data, label, _sort, color, fillColor) {
    ...
    const iqrLine = svg.children[1];
    const medianLine = svg.children[2];
    const meanLine = svg.children[3];
