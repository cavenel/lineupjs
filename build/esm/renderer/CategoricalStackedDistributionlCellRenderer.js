...
export default class CategoricalStackedDistributionlCellRenderer {
    constructor() {
...
function stackedBar(col, sanitize) {
    ...
    const bins = cats
        .map((c) => `<div class="${cssClass('distribution-bar')}" style="background-color: ${s(c.color)}; color: ${adaptTextColorToBgColor(c.color)}" title="${s(c.label)}: 0" data-cat="${s(c.name)}"><span>${s(c.label)}</span></div>`)
        .join('');
    return {
        template: `<div>${bins}`, // no closing div to be able to append things
        update: (n, hist, gHist) => {
            const bins = hist.hist.slice();
