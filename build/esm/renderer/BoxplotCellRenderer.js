...
export function computeLabel(col, v) {
    ...
    const mean = v.mean != null ? `mean = ${f(v.mean)} (dashed line)\n` : '';
    return `min = ${f(v.min)}\nq1 = ${f(v.q1)}\nmedian = ${f(v.median)}\n${mean}q3 = ${f(v.q3)}\nmax = ${f(v.max)}`;
}
export default class BoxplotCellRenderer {
    constructor() {
        this.title = 'Box Plot';
    }
    canRender(col, mode) {
    ...
    createSummary(col, context, _interactive, imposer) {
        return {
...
function renderDOMBoxPlot(col, n, data, label, sort, color, hasRange = false) {
    n.title = computeLabel(col, label);
    const whiskers = n.firstElementChild;
    const box = whiskers.firstElementChild;
    const median = box.nextElementSibling;
    const mean = whiskers.lastElementChild;
    ...
    // match lengths
    const outliers = Array.from(n.children).slice(1, hasRange ? -2 : undefined);
