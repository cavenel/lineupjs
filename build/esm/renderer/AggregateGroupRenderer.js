...
function matchNodes(node, length, clazz = 'agg-level', addTopN = false) {
...
function renderGroups(node, group, relativeIndex, col, provider) {
    ...
    const topNGetter = (group) => provider.getTopNAggregated(ranking, group);
    const isRow = relativeIndex >= 0;
    const isLeafGroup = !group.subGroups || group.subGroups.length === 0;
...
export default class AggregateGroupRenderer {
    constructor() {
        this.title = 'Default';
    }
    canRender(col) {
    ...
    createSummary(col, context) {
        return {
            ...
            update: (node) => {
                ...
                for (let i = 0; i < max; ++i) {
                    const child = children[i];
                    const subGroups = gparents.map((d) => (d[i] ? d[i].group : null)).filter((d) => d != null);
