...
export function matchColumns(node, columns, ctx) {
    ...
    columns.forEach((col) => {
        let childNode = node.querySelector(`[data-column-id="${col.column.id}"]`);
        if (!childNode) {
...
export function wideEnough(col, length = col.labels.length) {
...
// side effect
const adaptTextColorToBgColorCache = {};
/**
...
export const uniqueId = (function () {
    ...
    let idCounter = 0;
    return (prefix) => `${prefix}${(idCounter++).toString(36)}`;
})();
...
export function adaptColor(color, lightnessFactor = 1, saturationFactor = 1) {
